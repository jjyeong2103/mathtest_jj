...
if st.button('피드백 받기'):
    ...
    if len(label) >= 3:
        if label[0] == 1 and label[1] == 1 and label[2] == 1:
            ...
            st.success('거듭제곱의 거듭제곱, 거듭제곱의 곱셈을 이해하고 있구나! 일차방정식 풀이를 적용해서 풀이를 완성해보자!', icon="ℹ️")
        elif label[0] == 1:
            st.success('거듭제곱의 거듭제곱를 이해하고 있구나! 거듭제곱의 곱셈, 일차방정식 풀이를 적용해서 풀이를 완성해보자!', icon="ℹ️")
        elif label[1] == 1:
            st.success('거듭제곱의 곱셈을 이해하고 있구나! 거듭제곱의 거듭제곱, 일차방정식 풀이를 적용해서 풀이를 완성해보자!', icon="ℹ️")
        elif label[2] == 1:
            st.success('일차방정식 풀이를 이해하고 있구나! 거듭제곱의 거듭제곱, 거듭제곱의 곱셈을 적용해서 풀이를 완성해보자!', icon="ℹ️")
        else:
