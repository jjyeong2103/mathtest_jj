...
st.write("문제 풀이 방향이 크게 3가지 종류로 나누어질 수 있음. 실제로 학생들은 모범답안 1, 2의 방향으로의 풀이가 있고, 모범답안 3의 풀이는 없었음. 
풀이 방법에 따라 평가할 인지 요소의 종류 및 순서가 달라지는데, 이를 하나의 모델에 적용하는 것이 쉽지 않았던 것 같음.
")
...
st.write("예_1KB = 2^10B \\ 1MB = 2^10KB 용량 36MB \\ 1초당 9 \times 2^20B이기에 \left (2^10KB \right )^36 = 2^360(K^36)(B^36) \\ 
2^360(K^36)(B^36) \div 9 \times 2^20B = 9 \times 2^18(K^36)(B^17)초")
...
st.header("3-3")
st.write(r"**문제** : A \div 3y/2 = 4x^{2}y + 2xy + 6일 때. 다항식 A를 구하시오.")
st.write("**지식요소** : 등식의 성질, 다항식과 단항식의 곱셈, 단항식의 곱셈, 다항식과 단항식의 나눗셈, 단항식의 나눗셈")
latex_equation1 = r"""

st.write("**모범답안1**")
st.wirte("양변에 3y/2를 곱하여 곱셈으로 풀이")
data_3_3_1 = {
    '과정': ['A = (4x^{2}y + 2xy + 6) \times 3y/2', '= 4x^{2}y \times 3y/2 + 2xy \times 3y/2 + 6 \times 3y/2', '= 6x^{2}y^{2} + 3xy^{2} + 9y'],
    '지식요소': ['등식의 성질', '다항식과 단항식의 곱셈', '단항식과 단항식의 곱셈']
}
df_3_3_1 = pd.DataFrame(data_3_3_1)
st.table(df_3_3_1)
...
data_3_3_2 = {
    '과정': ['A \div 3y/2 = 4x^{2}y + 2xy + 6이므로', 'A = 6x^{2}y^{2} + 3xy^{2} + 9y'],
    '지식요소': ['다항식과 단항식의 나눗셈', '단항식과 단항식의 나눗셈']
}
df_3_3_2 = pd.DataFrame(data_3_3_2)
st.table(df_3_3_2)

st.write("**모범답안3**")
st.write("나눗셈을 역수의 곱셈으로 바꾸어 풀이")
data_3_3_3 = {
    '과정': ['A \div 3y/2 = A \times 2/3y= 4x^{2}y + 2xy + 6', 'A = 6x^{2}y^{2} + 3xy^{2} + 9y'],
    '지식요소': ['다항식과 단항식의 나눗셈', '다항식과 단항식의 곱셈, 단항식과 단항식의 곱셈']
}
df_3_3_3 = pd.DataFrame(data_3_3_3)
st.table(df_3_3_3)
...
st.write("예_A \div 3y/2 = 4x^{2}y + 2xy + 6= (4x^{2}y + 2xy + 6) \times 3y/2")
st.write("예_A \div 3y/2 = 4x^2y + 2xy + 6 =  6x^{2}y^{2} + 3xy^{2} + 9y")